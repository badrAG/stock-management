<template>
    <div>
     <div class="card-header mb-3"><h3> détails de marque </h3></div>
        <div class="row m-2">
            <div class="col-md-8">
            <h3> <span class="badge badge-secondary bdgtitle">{{count_agents}} Articles dans la marque {{ name }} </span></h3>
            </div>
...
export default {
    ...
    resultQuery(){
      ...
      return this.agents.filter((item)=>{
        ...
        methods:{
               editserv(id){
                 axios.get(`/api/marques/${this.$route.params.id}`)
          .then(res => {
