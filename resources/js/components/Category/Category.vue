<template>
    <div>
        <add-cat @added="catAdded"></add-cat>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
          <form @submit.prevent="updateCategory(names.id)" enctype="multipart/form-data">
          <div class="form-group">
        <input type="text" v-model="names.name" class="form-control mb-3" placeholder="Nom">
        </div>
     ...
     <div class="card-header mb-3"><h3>Les Categories</h3></div>
        <div class="row m-2">
            <div class="col-md-8">
                <button class="btn btn-primary ajoute" style=" width: 200px;" data-toggle="modal" data-target="#catModal"
                 type="button"> <i class="fas fa-plus m-2 "></i>Ajouter Un Catégorie</button>
            </div>
         ...
         <div class="col-md-12">
             ...
             <table class="table">
                 <thead class="thead-dark">
                     <tr>
                         <th class="p-2 text-center" scope="col"><span>Ref Catégorie</span></th>
                ...
                <thead v-if="resultQuery.length">
                    <tr v-for="(category,index) in resultQuery" :key="index">
                        <td  class="p-2 text-center">{{category.id}}</td>
                        <td  class="p-2 text-center" target="_blank">{{category.name}}</td>
                        <td  class="p-2 text-center">
                              <router-link  :to="{path : '/catégorie/details/'+category.id}">
                          <span class="text-primary m-2" data-toggle="tooltip" data-placement="top" title="Détails" style=" cursor: pointer;">
                                <i class="far fa-eye" ></i>
                             </span>
                        </router-link>
                            <span class="text-success m-2" data-toggle="modal" @click="editcat(category.id)" style="cursor: pointer;">
                                <i class="far fa-edit" data-toggle="tooltip" data-placement="top" title="Modifier"></i>
                                </span>
                            
                            <a @click="deleteArticle(category.id)">
            ...
            <td class="foot"></td>
                ...
                <pagination :data="categories" @pagination-change-page="getArticle"></pagination>
             </div>
            </div>
    </div>
...
import AddCat from './AddCatery.vue'
export default {
      data(){
         return{
             searchQuery:null,
      ...
      return this.categories.data.filter((item)=>{
        ...
        methods:{
            ...
            updateCategory(id){
                ...
                axios.post(`/api/category/${id}`,formData,config)
                .then(res => {
                 ...
                 axios.delete(`/api/category/${id}`).then(res=>{
                   ...
                   timer: 2500
                     ...
                     this.getArticle();
                        ...
                        icon: 'error',
                        title: 'Oops...',
                        text: 'Vous ne pouvez pas supprimer ce catégorie qui peut contenir certains articles!',
                        }));
                  }
                   })
            }
